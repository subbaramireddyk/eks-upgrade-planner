...
class K8sScanner:
    ...
    def scan_deployments(self, namespace: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            if namespace:
                items = self.k8s_helper.apps_v1.list_namespaced_deployment(
                    namespace
                ).items
            else:
                items = (
                    self.k8s_helper.apps_v1.list_deployment_for_all_namespaces().items
                )
    ...
    def scan_statefulsets(self, namespace: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            if namespace:
                items = self.k8s_helper.apps_v1.list_namespaced_stateful_set(
                    namespace
                ).items
            else:
                items = (
                    self.k8s_helper.apps_v1.list_stateful_set_for_all_namespaces().items
                )
    ...
    def scan_daemonsets(self, namespace: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            if namespace:
                items = self.k8s_helper.apps_v1.list_namespaced_daemon_set(
                    namespace
                ).items
            else:
                items = (
                    self.k8s_helper.apps_v1.list_daemon_set_for_all_namespaces().items
                )
    ...
    def scan_crds(self) -> List[Dict[str, Any]]:
        ...
        try:
            items = (
                self.k8s_helper.apiextensions_v1.list_custom_resource_definition().items
            )
    ...
    def scan_ingresses(self, namespace: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            if namespace:
                items = self.k8s_helper.networking_v1.list_namespaced_ingress(
                    namespace
                ).items
            else:
                items = (
                    self.k8s_helper.networking_v1.list_ingress_for_all_namespaces().items
                )
    ...
    def detect_deprecated_apis(
        ...
        if deprecated_resources:
            logger.warning(
                f"Found {len(deprecated_resources)} deprecated API versions in use"
            )
        else:
