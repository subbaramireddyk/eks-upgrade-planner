...
class RiskAssessment:
    ...
    def assess_deprecated_api_risk(
        ...
        removed_apis = sum(
            1 for api_version, resources in deprecated_apis.items()
    ...
    def assess_addon_compatibility_risk(
        ...
        """
        incompatible_addons = [
            a for a in addon_recommendations
            if not a.get('is_compatible', True)
        ]
        ...
        critical_incompatible = [
            a for a in incompatible_addons
            if a.get('addon_name') in critical_addons
        ]
    ...
    def perform_comprehensive_assessment(
        ...
        if api_risk['risk_level'] in ['HIGH', 'CRITICAL']:
            ...
            mitigation_strategies.append("Update all deprecated API usage before upgrade")
        
        if breaking_risk['risk_level'] in ['MEDIUM', 'HIGH']:
            risk_factors.append(f"⚠️  {breaking_risk['message']}")
            mitigation_strategies.append("Review and test all breaking changes in staging")
        
        if addon_risk['risk_level'] in ['MEDIUM', 'HIGH']:
            risk_factors.append(f"⚠️  {addon_risk['message']}")
            mitigation_strategies.append("Upgrade incompatible addons before EKS upgrade")
        
        if version_risk['version_jumps'] > 1:
            risk_factors.append(f"ℹ️  {version_risk['message']}")
            mitigation_strategies.append("Plan for sequential upgrades with validation between")
        ...
        downtime_estimate = self._estimate_downtime(
            overall_risk,
            version_risk['version_jumps'],
            size_risk['total_nodes']
        )
    ...
    def generate_risk_summary(self, assessment: Dict[str, Any]) -> str:
        ...
        lines.append(f"Estimated Downtime: {assessment['estimated_downtime']['recommended_window']}")
