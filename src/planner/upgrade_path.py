...
class UpgradePathPlanner:
    ...
    def create_upgrade_runbook(
        ...
        pre_eks_addons = [a for a in ordered_addons if a.get('upgrade_timing') == 'before_eks']
        
        if pre_eks_addons:
            addon_steps = []
            for addon in pre_eks_addons:
                addon_steps.append(
                    f"Upgrade {addon.get('name')} to compatible version"
                )
        ...
        post_eks_addons = [a for a in ordered_addons if a.get('upgrade_timing') == 'after_eks']
