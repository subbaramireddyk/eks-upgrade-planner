...
class JSONExporter:
    ...
    def export_report(
        ...
        report = {
            ...
            'cluster': {
                ...
                'target_version': upgrade_plan.get('upgrade_path', [])[-1] if upgrade_plan.get('upgrade_path') else None,
                'region': cluster_info.get('cluster', {}).get('vpc_config', {}).get('vpcId'),
                'status': cluster_info.get('cluster', {}).get('status'),
            },
            'scan_results': scan_results,
            'analysis': {
                'compatibility': {
                    'addon_recommendations': analysis_results.get('addon_recommendations', []),
                    'version_compatibility': analysis_results.get('version_compatibility', {}),
                },
                'deprecations': {
    ...
    def export_summary(
        ...
        summary = {
            ...
            'estimated_hours': upgrade_plan.get('time_estimation', {}).get('total_hours'),
            'node_groups': len(cluster_info.get('node_groups', [])),
            'addons': len(cluster_info.get('addons', [])),
            'deprecated_api_count': len(risk_assessment.get('individual_assessments', {}).get('deprecated_apis', {}).get('deprecated_apis', {})),
            'breaking_changes_count': len(risk_assessment.get('individual_assessments', {}).get('breaking_changes', {}).get('breaking_changes', [])),
        }
