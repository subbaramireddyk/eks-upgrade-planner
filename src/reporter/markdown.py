...
class MarkdownReporter:
    ...
    def generate_report(
        ...
        sections.append(self._generate_pre_upgrade_requirements(
            upgrade_plan, analysis_results
        ))
...
**Target Version:** {target_version}"""
    ...
    def _generate_executive_summary(
        ...
        downtime = risk_assessment.get('estimated_downtime', {}).get('recommended_window', 'Unknown')
...
{self._format_addons(addons)}"""
    ...
    def _format_node_groups(self, node_groups: List[Dict[str, Any]]) -> str:
        ...
        lines = ["| Name | Version | AMI Type | Capacity | Instance Types |", "| --- | --- | --- | --- | --- |"]
        ...
        for ng in node_groups:
            ...
            lines.append(f"| {name} | {version} | {ami_type} | {desired} | {instances} |")
    ...
    def _generate_pre_upgrade_requirements(
        ...
        for addon in addon_recs:
            if addon.get('action_required'):
                ...
                lines.append(f"- [ ] Update {addon.get('addon_name')}: {current} → {recommended}")
...
- Document all changes made during upgrade"""
    ...
    def _generate_timeline(self, upgrade_plan: Dict[str, Any]) -> str:
        ...
        lines.append(f"**Recommended Maintenance Window:** {time_est.get('recommended_window', 'Unknown')}")
