...
def scan(ctx, cluster, region, profile):
    ...
    try:
        ...
        try:
            ...
            click.echo(f"  StatefulSets: {k8s_results['summary']['total_statefulsets']}")
...
@click.option('--profile', help='AWS profile name')
@click.option('--target-version', required=True, help='Target EKS version')
@click.option('--format', type=click.Choice(['markdown', 'json', 'html']), default='markdown', help='Output format')
